...
echo " No.of files in the current directory is : $(ls | wc -l)" # ls =  command used for long listing and wc = command for word counting , (|) = this is known as piping when we want command line argument from previous command out put then we use piping
...
echo " The no.of lines in current file is : $(cat var_comm_subst.sh | wc -l)" # cat command used for view or edit content of any file. here cat var_comm_subst.sh command will show content of this file and by using piping we count no.of line  
